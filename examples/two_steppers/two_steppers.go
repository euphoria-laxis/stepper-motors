...
stepper 2 in 1	->		|   5 | GPIO.21 |   IN | 0 | 29 || 30 |   |      | 0v      |     |
stepper 2 in 2	->		|   6 | GPIO.22 |   IN | 0 | 31 || 32 | 0 | IN   | GPIO.26 | 12  |
stepper 2 in 3	->		|  13 | GPIO.23 |   IN | 0 | 33 || 34 |   |      | 0v      |     |
stepper 2 in 4	->		|  19 | GPIO.24 |   IN | 0 | 35 || 36 | 0 | IN   | GPIO.27 | 16  |
...
// GPIO 27  |    13
// GPIO 22  |    15
var sm1gpios = [4]int{17, 18, 27, 22} // Stepper motor GPIOs position (BCM)
...
// GPIO 23  |    33
// GPIO 24  |    35
var sm2gpios = [4]int{5, 6, 13, 19} // Stepper motor GPIOs position (BCM)
...
func main() {
	...
	// Create stepper.StepperMotor instances
	sm1 := stepper.NewStepperMotor(
		stepper.SetGPIOs(sm1gpios),
	)
	sm2 := stepper.NewStepperMotor(
		stepper.SetGPIOs(sm2gpios),
	)
	...
	sm2Params := params{
		direction: stepper.DirectionCounterClock, // direction in which the stepper motor will rotate
		speed:     stepper.Speed60,               // stepper motor speed (valid speeds are 20%, 40%, 60%, 80% and 100%)
		angle:     270,                           // will rotate to 270° from initial position
		duration:  2000 * time.Millisecond,       // time to wait after running
	}

	// Run stepper motor 1 in a routine
	var wg sync.WaitGroup
	wg.Add(1)
	go func(sm *stepper.StepperMotor, p *params) {
		defer wg.Done()
		runStepperMotor(sm, p)
	}(sm1, &sm1Params)
	// Run stepper motor 2 in a routine
	wg.Add(1)
	go func(sm *stepper.StepperMotor, p *params) {
		defer wg.Done()
		runStepperMotor(sm, p)
